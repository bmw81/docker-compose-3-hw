...
`1. docker compose run -e MY_PASS=... (передача через -e в командной строке)`

`2. export MY_PASS=... и затем docker compose up`

`3. environment: в compose-файле`

`4. --env-file в командной строке`

`5. env_file: в compose-файле`

`6. .env файл в директории проекта`
...
`Анонимные тома для идентификации имеют неудобочитаемое хэш-имя`

`При docker compose down -v они удаляются все разом, включая те, которые, возможно, были нужны`

`Именованный том с явным именем (writer_data) остаётся после down и не удаляется даже с -v, пока его явно не удалить через docker volume rm.`

`Для продакшена (БД): Только именованный том. Он не удалится случайно при пересоздании стека, его легко бэкапировать по имени.`
...
`Разница: CMD — это аргументы по умолчанию для ENTRYPOINT. Если ENTRYPOINT переопределён как ["sleep"], то CMD всё ещё передаётся как аргумент. Если ENTRYPOINT уже содержит аргументы (["sleep", "9999"]), то CMD игнорируется, потому что ENTRYPOINT уже "укомплектован".`
...
`Значение изменилось. Скалярные значения (строки) всегда заменяются при merge, а не merge'ятся.`
